...
We want to get top 3 silos and compare them in terms of manually assigned terms from the original map.
Also compare author affiliations (country).

```{r extract components, eval = TRUE}
#str(g1)
...
sort_desc(c$csize)[1:3] #sizes of the top 3 components
...
NOTE: 4th largest component has 57 authors - might be worth exploring too?


```{r plot top3 components, eval = TRUE}

pdf(here("lens-results", "components_top3.pdf"), width = 8, heigh = 8, pointsize = 18)
...
```{r analyse top3 components, eval = TRUE}
...
authors_comp_top3_pubids_basic <- bind_rows(authors_comp_308_pubids_basic, authors_comp_164_pubids_basic, authors_comp_98_pubids_basic, .id = "Comp")
#str(authors_comp_top3_pubids_basic)

#histogram of publication years
ggplot(authors_comp_top3_pubids_basic, aes(year, fill = Comp)) + geom_histogram(binwidth = 5)
#with feqpolygons for better readability
ggplot(authors_comp_top3_pubids_basic, aes(year, colour = Comp)) + geom_freqpoly(binwidth = 5)
...
```{r compare components  terms, eval = TRUE}
...
dim(myterms)
#names(myterms)
myterms$title <- tolower(myterms$Title) #make to lowercase to make it easier to merge by title (using fuzzyjoin package)
...
p_wordfreq1 <- as.data.frame(table(data_comp_308_terms)) %>% #count terms and create data frame
  ...
  rename(Terms = data_comp_308_terms) %>% #rename column
	ggplot(aes(x = reorder(Terms,(Freq)), y =  (Freq)/sum(Freq), fill = Terms)) + 
...
p_wordfreq1 / p_wordfreq2 / p_wordfreq3 +
  plot_layout(ncol = 3, nrow = 1, widths = c(1, 1, 1)) +
  plot_annotation(tag_levels = "A") &
  theme(plot.tag.position = c(0, 0.98), plot.tag = element_text(size = 14, face = "bold"))

#save the figure 
ggsave(here("lens-results", "components_top_terms.pdf"), width = 16, height = 8, units = "cm", dpi = 300, scale = 2)

#TODO - fix few colors in the panel C
```


## Get author affiliations by component

```{r compare top3 components authors, eval = TRUE}
...
#get publications for component with 308 authors (component 3):
record_df_data.authors.ids %>% 
  filter(data.lens_id %in% unique(authors_comp_308$pub.id)) -> authors_comp_308_pubs #publications of the authors flom component 3
#str(authors_comp_308_pubs)

# publication details for component with 308 authors (component 3):
...
#get affiliations for authors for component with 308 authors (component 3):
dt %>%
...
length(unique(authors_comp_308_aff$Author)) #200 unique authors captured
table(authors_comp_308_aff$country_code) #mostly GB (but not taking into account that same authors appear multiple times)

#show collapsed country codes for component with 308 authors (component 3):
authors_comp_308_aff %>% 
  group_by(Author) %>%
  summarize(country_code = paste(country_code, collapse = "; ")) 

#only keep the first listed country code and retain more columns for component with 308 authors (component 3)::
authors_comp_308_aff %>% 
  group_by(Author) %>%
  summarize(country_code = first(country_code), affiliation = first(name)) -> authors_comp_308_oneaff

#make a barplot of country codes for component with 308 authors (component 3):
p_countryfreq1 <- as.data.frame(table(authors_comp_308_oneaff$country_code)) %>% #count terms and create data frame
  #top_n(10) %>% #pick top 10
  rename(Codes = Var1) %>% #rename column
  #rbind(setNames(data.frame("Pollution", 0), c("Codes", "Freq"))) %>%
...
#get publications for component with 164 authors (component 3):
record_df_data.authors.ids %>% 
  filter(data.lens_id %in% unique(authors_comp_164$pub.id)) -> authors_comp_164_pubs #publications of the authors flom component 3
#str(authors_comp_164_pubs)

# publication details for component with 164 authors (component 3):
authors_comp_164_pubs %>% 
  group_by(data.lens_id) %>%
  summarize(Paper_title = data.title, publication_type = data.publication_type, Year = data.year_published , Authors = paste(Author, collapse = "; "), Author_id = paste(value, collapse = "; ")) %>% 
  distinct_all() -> authors_comp_164_pubs_details

#get affiliations for authors for component with 164 authors (component 3):
dt %>%
...
#show collapsed country codes for component with 164 authors (component 3):
authors_comp_164_aff %>% 
  group_by(Author) %>%
  summarize(country_code = paste(country_code, collapse = "; ")) 

#only keep the first listed country code and retain more columns for component with 164 authors (component 3)::
authors_comp_164_aff %>% 
  group_by(Author) %>%
  summarize(country_code = first(country_code), affiliation = first(name)) -> authors_comp_164_oneaff

#make a barplot of country codes for component with 164 authors (component 3):
p_countryfreq2 <- as.data.frame(table(authors_comp_164_oneaff$country_code)) %>% #count terms and create data frame
...
#get publications for component with 98 authors (component 3):
record_df_data.authors.ids %>% 
  filter(data.lens_id %in% unique(authors_comp_98$pub.id)) -> authors_comp_98_pubs #publications of the authors flom component 3
#str(authors_comp_98_pubs)

# publication details for component with 98 authors (component 3):
authors_comp_98_pubs %>% 
  group_by(data.lens_id) %>%
  summarize(Paper_title = data.title, publication_type = data.publication_type, Year = data.year_published , Authors = paste(Author, collapse = "; "), Author_id = paste(value, collapse = "; ")) %>% 
  distinct_all() -> authors_comp_98_pubs_details

#get affiliations for authors for component with 98 authors (component 3):
dt %>%
...
#show collapsed country codes for component with 98 authors (component 3):
authors_comp_98_aff %>% 
  group_by(Author) %>%
  summarize(country_code = paste(country_code, collapse = "; ")) 

#only keep the first listed country code and retain more columns for component with 98 authors (component 3)::
authors_comp_98_aff %>% 
  group_by(Author) %>%
  summarize(country_code = first(country_code), affiliation = first(name)) -> authors_comp_98_oneaff

#make a barplot of country codes for component with 98 authors (component 3):
p_countryfreq3 <- as.data.frame(table(authors_comp_98_oneaff$country_code)) %>% #count terms and create data frame
  #top_n(10) %>% #pick top 10 terms - there are only 6 terms!
  rename(Codes = Var1) %>% #rename column
  #rbind(setNames(data.frame("Pollution", 0), c("Codes", "Freq"))) %>% #add 7th term with value 0
  ggplot(aes(x = reorder(Codes,(Freq)), y =  (Freq)/sum(Freq), fill = Codes)) + 
  geom_bar(stat = 'identity') +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  coord_flip() +
...
#Assemble into a single multi-panel plot:
p_countryfreq1 / p_countryfreq2 / p_countryfreq3 +
  plot_layout(ncol = 3, nrow = 1, widths = c(1, 1, 1)) +
  plot_annotation(tag_levels = "A") &
  theme(plot.tag.position = c(0, 0.98), plot.tag = element_text(size = 14, face = "bold"))
...
authors_central <- g1_degree[g1_degree > 10] #10 people for >25, 59 for > 15, 163 for >10
...
#length(authors_central) #163
#dim(authors_central_table) #160
...
#check cluster numbers - for recoding to 1 2 3 - used below
# as.numeric(c$membership[unique(authors_central_aff$Author)]) #2 5 20
# which(c$csize == 308) #2
# which(c$csize == 164) #5
# which(c$csize == 98) #20
authors_central_table$component_nr <- case_when(authors_central_table$component_orig == 2 ~ 1,
                                         ...
                                         ) #we only assign numbers 1,2,3 to members of the three biggest clusters, rest is NA
...
#TODO 
# colour plot by country affiliation (most recent)
# maybe different colours on the network plot? add names on the network plot?
# overlay country affiliation on the network plot (all authors?)


## Plot author collaboration network coloured by suthor affiliation country
...
record_df_data.authors.aff <- read.csv(here("data", "record_df_data.authors.aff_cleaned.csv"))
record_df_data.authors.aff$Author <- paste(record_df_data.authors.aff$last_name, record_df_data.authors.aff$initials, sep = ", ")
#get affiliations of unoque authors
record_df_data.authors.aff %>% 
...
## Generate colors for country codes:
...
levels(authors_all_aff$country_color) <- unname(mycolors) #assign colors from teh papette
...
```{r plot top3 components and countries, eval = TRUE}

pdf(here("lens-results", "components_top3_countries.pdf"), width = 16, heigh = 8, pointsize = 16)
par(mfrow=c(1,2), mar = c(0,0,1,0))

plot(g1, layout = g1_graph_nicely, vertex.size = 5, vertex.label = NA, vertex.color = adjustcolor(V(g1)$color, alpha.f = .8))
text(0, 1.1, label = "A. Three largest network components ")
