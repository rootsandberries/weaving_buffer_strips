...
## Summary
This code starts from the list of the 1019 included articles stored in **20201215_EGM_Net_all-articles_clean.csv**. This list was generated from the original systematic map (Haddaway et al. 2018), with basic data cleaning steps carried out in OpenRefine. The code runs a query via LENS API custom function to retrieve the detailed bibliographic information of each record. Retrieved data is processed to remove duplicated records, resulting in 974 records with unique titles found in LENS. The records were then manually inspected and false positives (e.g. errata, commentary, etc.) were removed for a total of 936 records. Preliminary summary plots are made for the 864 records that contain lists of references represented as LENS reference ID. Out of 936 records, 859 are journal articles, 752 have DOIs, and publication years span 1954 - 2018.  The data is stored as a hierarchical data object with multiple levels of nested data tables (**record_df** stored in **LENS_dataframe.RData**). Subsequent code is used to extract and flatten the selected data structures, remove redundancies and resolve conflicting information on the authors and their countries of affiliation, with a few duplicated articles removed.    
...
# length(unique(dat$DOI)) #only 184 DOI values and missing values are stored as ""! (change to NA later)
...
*DO NOT RUN* - the output was saved to a file at the end of this chunk - this output is loaded at the start of the next chunk, to save long waiting time during LENS API query.   

```{r query LENS for full records, eval = FALSE}
#subset of first 50  publications, (used for testing)
#dat50 <- dat[1:50, ] # use instead of "dat" in the code below
...
#prepare request
#max_results <- 500 #to limit number of records per request, max currently allowed is now 500
...
#Visual check - some records have more info than others
#View(record_df[duplicated(record_df$data.title) | duplicated(record_df$data.title, fromLast=TRUE), ]) 
...
#Visual inspection of new dataframe
par(mar=c(4,15,2,2))
...
dim(record_df_data.authors) #3461 - one row for each author (but authors can have multiple IDs and affiliations - nested in author name)
...
dim(record_df_data.authors.ids) #4015 - one row for each author ID
...
#example of cleaning these using code, one by one 
